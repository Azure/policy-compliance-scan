...
function triggerScan(scope, token) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        return httpClient_1.sendRequest(webRequest).then((response) => {
            if (response.headers['location']) {
                let pollLocation = response.headers['location'];
                utilities_1.printPartitionedText(`Scan triggered successfully.\nPoll URL: ${pollLocation}`);
        ...
        try {
            ...
            while (pendingPolls.length > 0) {
                ...
                for (const poll of pendingPolls) {
                    const isCompleted = yield isScanCompleted(poll.location, token);
                    if (isCompleted) {
                ...
                if (completedPolls.length > 0) {
                    ...
                    endTime = new Date();
                    utilities_1.printPartitionedDebugLog(`Results saved. Time taken in ms:: ${endTime.getTime() - startTime.getTime()}`);
                }
